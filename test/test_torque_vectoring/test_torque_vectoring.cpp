...
static TVInput straight() {
    ...
    return TVInput{ Ampere{20.0f}, DegPerSec{}, Unit{}, Mps{},
...
void test_full_bringup_demand_is_500_a_per_motor_when_tv_off(void) {
    TVYawState s{};
    TVInput in = straight();
    in.total_torque = Ampere{1000.0f};
...
void test_intermediates_are_populated(void) {
    ...
    TVYawState s{};
    TVOutput o = tv_compute(straight(), s);
    TEST_ASSERT_TRUE((float)o.fz_L > 0.0f && (float)o.fz_R > 0.0f);   // 정적하중은 양수
    TEST_ASSERT_TRUE((float)o.max_torque_L > 0.0f);     // 트랙션 상한 존재
}
...
void test_low_speed_explicitly_disables_yaw_feedback(void) {
    ...
    s.integral = 50.0f;
    TVInput in = straight();
    in.yaw_rate = DegPerSec{20.0f};
    in.vehicle_speed = Mps{0.5f};
    TVOutput o = tv_compute(in, s);
    TEST_ASSERT_FLOAT_WITHIN(0.001f, 0.0f, (float)o.yaw_moment);
...
void test_zero_gains_are_strict_5050_off_even_beyond_friction_model(void) {
    TVYawState s{};
    TVInput in = straight();
    in.vehicle_speed = Mps{15.0f};
    in.steering_angle = Unit{0.8f};
    in.yaw_rate = DegPerSec{-30.0f};
    in.ay = GForce{10.0f}; // deliberately drives the Stage-4 diagnostic limit to zero
    TVOutput o = tv_compute(in, s);
