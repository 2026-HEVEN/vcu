...
void test_zero_and_low_speed_are_disabled() {
    TEST_ASSERT_FLOAT_WITHIN(0.001f, 0.0f,
        (float)tv_reference_compute(Unit{0.5f}, Mps{0.5f}, TV_PARAMS));
}

void test_steering_sign_and_symmetry() {
    const float left = (float)tv_reference_compute(Unit{0.25f}, Mps{5.0f}, TV_PARAMS);
    const float right = (float)tv_reference_compute(Unit{-0.25f}, Mps{5.0f}, TV_PARAMS);
    TEST_ASSERT_TRUE(left > 0.0f);
    TEST_ASSERT_FLOAT_WITHIN(0.001f, left, -right);
}

void test_target_is_limited() {
    const float yaw = (float)tv_reference_compute(Unit{1.0f}, Mps{30.0f}, TV_PARAMS);
...
void test_friction_limit_caps_high_speed_target() {
    ...
    const float yaw = (float)tv_reference_compute(Unit{1.0f}, Mps{30.0f}, p);
...
void test_invalid_speed_is_safe() {
    ...
    TEST_ASSERT_FLOAT_WITHIN(0.001f, 0.0f,
        (float)tv_reference_compute(Unit{0.5f}, Mps{nan_value}, TV_PARAMS));
}
