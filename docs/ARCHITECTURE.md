...
- 기어 ADC 판정과 100 ms 안정화는 구현돼 있지만, PCB V3 GPIO32 전압을 실측해야 한다.
  `GEAR_SELECTOR_INSTALLED=true`로 GPIO32 기어 입력을 사용하며, 정지·스로틀 해제 인터록 뒤 안정적으로 판정된 D/R 방향만 구동한다.
- 브레이크 압력과 LV 전압은 PCB V3 핀이 정의됐지만 아직 읽지 않는다. 센서 스케일링과
  CAN ID가 확정된 뒤 구현한다.
