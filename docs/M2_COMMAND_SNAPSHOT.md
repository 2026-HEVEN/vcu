...
스냅샷은 좌우 전류·기어·구동 허용 상태를 한 묶음으로 복사한 명령이다.
복사 자체와 별개로, 너무 오래된 명령은 사용하지 않도록 명령의 나이를 검사한다.

- 잘못된 순서: 시계 읽기(1000ms) → 새 명령 게시(1001ms) → 그 명령 복사.
  옛 시각으로 계산하면 `1000 - 1001`이 된다. 부호 없는 32비트 정수에서는
  -1이 아니라 4,294,967,295ms가 되어, 방금 만든 명령을 오래된 것으로 오인한다.
- 올바른 순서: 명령 복사(게시 시각 1001ms) → 시계 읽기(1002ms).
  `1002 - 1001 = 1ms`이므로 정상적으로 최신 명령으로 판정한다.

코드는 **① 명령 복사 → ② 검사 시각 읽기 → ③ 유효기간 검사** 순서다.
제어 주기나 전류 크기를 바꾸는 수정이 아니라, 나이를 재는 기준 시각을 바로잡는 수정이다.
타이머 자체가 최댓값에서 0으로 돌아가는 정상 rollover 처리는 그대로 유지한다.

### 세부 변경 사항

- 신선도 계산용 시간은 스냅샷 복사 **이후** 읽는다. 복사 전 now를 사용하면
  중간 재연결 시리얼 출력 등의 yield 동안 게시된 새 timestamp와 unsigned
  뺄셈이 뒤집혀 정상 명령이 HALT될 수 있었다. 제어 heartbeat도 읽은 뒤 시간을 잰다.
- seq 증가가 0으로 wrap되면 1로 진행해 '미게시' 상태로 오인하지 않는다.
- `send_torque` 성공은 **TWAI 큐 삽입 성공**이다. 실제 전송 완료나 MCU 수신을
  증명하지 않는다. 미핸드셰이크는 성공이 아닌 `Skipped`로 기록한다.
- `MotorTxDiagnostics`는 요청 좌우 값/seq, 신선도, 측별 결과와 누적 실패,
  마지막 큐 성공 명령·시각·seq를 저장하며 같은 mux로 원자 게시/복사한다.
- 실패 시 마지막 큐 성공값을 보존한다. 0A로 덮으면 '0A 전송 성공'처럼 보이므로
  하지 않는다. 성공 시 연속 실패만 초기화, 누적 실패는 uint32 포화로 유지한다.
- 1Hz `MOTOR_TX`: result=0 미시도/1 큐 성공/2 실패. queuedValid가 false인
  queuedA/queuedSeq는 미수신 초기값이며 queuedAge=4294967295로 표시한다.
  `snapAge/fresh`는 해당 TX 주기의 판정, `staleTicks`는 stale/미게시 주기 누적이다.
  queuedAge는 읽기 시점 기준이며 큐 지연/컨트롤러 동작 시각이 아니다.
- 0x1C01C0D0 로그는 계속 **요청 전류**를 보낸다. 좌우 요청값은 같은 진단
  복사본에서 가져온다. 기존 CAN 바이트 계약은 변경하지 않았다. 큐 성공 상태는
  시리얼 로그에서 확인하며, Monolith 단독으로 확인하려면 별도 CAN 확장이 필요하다.
- 실패 3회 링크 무효화/기존 복구 램프 정책은 유지. 실제 송신 실패 시험은 별도다.
- 기본 Arduino 설정은 loop core=1이고 life task도 core=1이다. 아래 초기 문서의
  '항상 서로 다른 코어' 설명은 정확하지 않다. 태스크 전환/yield 및 향후 코어 분리를
  모두 고려해 같은 mux 보호를 유지한다.
- native 회귀 검증: 시각 경계·rollover·미게시, 성공/실패/미시도 구분,
  마지막 성공값 보존·카운터 포화·좌우 비대칭 송신 결과를 추가했다.

이번 수정은 기존 회생 정책/전력 제한/MCU CAN 포맷을 바꾸지 않는다.
로컬 회생 및 energymeter-jys 작업은 별도로 통합해야 한다.
