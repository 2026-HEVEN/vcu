...
## 최신 dev 통합 (2026-09-21)

dev c76f0e9를 병합하여 회생 부호/페달 정책을 포함한다. 아래 초기 기준보다 우선한다.
브레이크+양수 스로틀도 propulsion_requested=true이므로 EM 무효 시 D/R 구동은
모두 차단되고, 정상 시 기존 가속 램프가 유지된다. 스로틀 0%에서만 D 회생을
허용하는 dev 정책을 유지하며 회생/EM 제어 활성 플래그는 여전히 OFF다.
스냅샷 PR #28은 이 브랜치에 병합하지 않았다.
