...
- 시리얼 모터 펄스 명령 상한: 모터당 50 A, 100~300 ms
...
5. `MOTOR_L 50 300`, `MOTOR_R 50 300`, `MOTOR_BOTH 50 300` 순서로 한 번씩 실행한다.
6. `FAST_CSV`에서 명령/상전류/RPM/WSS와 CAN 오류 누적 여부를 비교한다.

CAN 오류 카운터가 증가하거나 WSS 채널과 실제 바퀴 위치가 다르면 반복 구동하지
말고 배선과 종단저항부터 확인한다. 이 브랜치에서는 50 A보다 높일 수 없다.
