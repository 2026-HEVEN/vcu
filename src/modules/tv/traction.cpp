...
MaxTorque tv_traction_compute(WheelLoads fz, GForce ay_q, const TVParams &p) {
    const float fz_l_in = (float)fz.fz_L;
    const float fz_r_in = (float)fz.fz_R;
    const float ay_g = (float)ay_q;
    if (!std::isfinite(fz_l_in) || !std::isfinite(fz_r_in) ||
        !std::isfinite(ay_g) || p.mu <= 0.0f || p.tire_radius_m <= 0.0f ||
        p.gear_ratio <= 0.0f || p.motor_kt_nm_per_a <= 0.0f ||
        p.motor_current_max_a <= 0.0f) return {Ampere{}, Ampere{}};

    const float left_fz = fz_l_in > 0.0f ? fz_l_in : 0.0f;
    const float right_fz = fz_r_in > 0.0f ? fz_r_in : 0.0f;
    const float rear_fz = left_fz + right_fz;
    if (rear_fz <= 0.0f) return {Ampere{}, Ampere{}};
    ...
    return {Ampere{current_limit(left_fz, fy_left)},
            Ampere{current_limit(right_fz, fy_right)}};
}
