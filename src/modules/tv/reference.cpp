...
DegPerSec tv_reference_compute(Unit steering, Mps speed_mps, const TVParams &p) {
    const float speed_in = (float)speed_mps;
    if (!std::isfinite(speed_in) || !std::isfinite((float)steering) ||
        p.wheelbase_m <= 0.0f || p.desired_yaw_max <= 0.0f) return DegPerSec{};

    const float speed = std::fabs(speed_in);
    if (speed < p.tv_min_speed_mps) return DegPerSec{};

    const float delta = (float)steering * p.max_steer_rad;
    const float denominator = p.wheelbase_m + p.understeer_grad * speed * speed;
    if (denominator <= 1.0e-6f) return DegPerSec{};
    ...
    return DegPerSec{clampf(desired_radps * RAD_TO_DEG,
